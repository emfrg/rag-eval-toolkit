...
class RAGSystem(BaseRAGSystem):
    def __init__(self, config: RAGConfig, dataset: Optional[RAGDataset] = None) -> None:
        self._impl = self._create_impl(config, dataset)
        super().__init__(config, dataset)

    @property
    def backend(self) -> BaseRAGSystem:
        return self._impl

    @property
    def index_report(self) -> Optional[IndexReport]:
        return getattr(self._impl, "index_report", None)

    def build_index(self, dataset: RAGDataset) -> IndexReport:
        return self._impl.build_index(dataset)

    def retrieve(self, question: str) -> List[Document]:
        return self._impl.retrieve(question)

    def query(self, question: str) -> Tuple[str, List[Document]]:
        return self._impl.query(question)

    def _create_impl(
        self, config: RAGConfig, dataset: Optional[RAGDataset]
    ) -> BaseRAGSystem:
        if config.rag_model == "naive":
            return NaiveRAGSystem(config, dataset)
        if config.rag_model == "graphrag":
            try:
                from .graphrag import LightRAGSystem
            except ModuleNotFoundError as exc:  # pragma: no cover
                raise RuntimeError(
                    "LightRAG dependencies are missing. Install the lightrag package."
                ) from exc
            return LightRAGSystem(config, dataset)
        raise ValueError(f"Unknown rag_model '{config.rag_model}'")
