...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True),
    help="Path to configs JSON file (array of configs)",
)
@click.option(
    "--output",
    "-o",
...
@click.option(
    ...
    type=int,
    default=None,
    help="Questions per batch (default: 40 from config)",
)
@click.option(
    "--max-workers",
    type=int,
    default=None,
    help="Max concurrent workers (default: 15 from config)",
)
@click.option(
    "--recreate",
    is_flag=True,
    help="Force fresh start, ignoring any existing checkpoint",
)
@click.option(
    "--no-tracking",
    is_flag=True,
...
def run(
    ...
    Examples:
        ...
        # Run configs
        rag-eval run -c corpus.jsonl -d questions.jsonl -C configs/naive.json

        # Run on a sample of 10 questions
        rag-eval run -c corpus.jsonl -d questions.jsonl -C configs/naive.json -n 10
    """
    ...
    # Load configs (always array format)
    ...
    # Override eval config from CLI options (if provided)
    if batch_size is not None or max_workers is not None:
        ...
        console.print(f"[blue]Eval config: batch_size={batch_size}, max_workers={max_workers}[/blue]")

    # Run experiment
    console.print("\n[green]Starting evaluation...[/green]\n")
    runner = ExperimentRunner(output_dir=output)
    summary = runner.run_experiments(
        rag_configs, corpus_data, eval_dataset, recreate=recreate, tracking=not no_tracking, experiment_name=experiment
    )
    ...
    for result in summary.results:
        # Use config name if available, otherwise use config_sig
        config_name = result.config.get("name", result.config_sig[:8])
        row = [
            config_name,
    ...
    if best:
        best_name = best.config.get("name", best.config_sig)
        console.print(f"\n[green]Best config (by faithfulness): {best_name}[/green]")
